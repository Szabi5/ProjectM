...
//
// This is your original rollover_tab layout preserved, with only the export flow changed:
// - The export now saves the .xlsx to Downloads (or fallback) first.
// - After export the UI presents an AlertDialog where you can Open the file or Share it.
// - Uses rollover_export helper (exportAndSaveRolloverExcel) and open_file + share_plus for actions.
//
...
class _RolloverTabState extends State<RolloverTab> {
  ...
  // Updated export flow: save file first, then show options to Open or Share
  Future<void> _exportRolloverExcel() async {
    ...
    try {
      ...
      // Save file to Downloads (or fallback) and get path
      ...
      // Show dialog with actions: Open, Share, Dismiss
      if (!mounted) return;
      showDialog<void>(
        context: context,
        builder: (_) {
...
// Reusable UI components follow (same as your template)
