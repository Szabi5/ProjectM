...
class EuMortgageScreen extends StatefulWidget {
  const EuMortgageScreen({super.key});
  @override
  State<EuMortgageScreen> createState() => _EuMortgageScreenState();
}

class _EuMortgageScreenState extends State<EuMortgageScreen> {
  ...
  late final Map<String, TextEditingController> _controllers = {
    ...
    'extraPct': extraPctController,
    'lumpSum': lumpSumController,
    'lumpMonth': lumpMonthController,
    'oneOffSum': oneOffSumController,
    'oneOffMonth': oneOffMonthController,
    'inflation': inflationController,
    'rateChanges': rateChangesController,
  };
  ...
  void _saveSettings() async {
    ...
    for (var entry in _controllers.entries) {
      await prefs.setString('eu_mortgage_${entry.key}', entry.value.text);
    }
    setState(() {
  ...
  void _loadSettings() async {
    ...
    for (var entry in _controllers.entries) {
      final value = prefs.getString('eu_mortgage_${entry.key}') ?? defaults[entry.key] ?? '';
  ...
  @override
  void dispose() {
    _controllers.values.forEach((c) => c.dispose()); // Simplified dispose
    super.dispose();
  }
  ...
  void _simulate() async {
    ...
    // --- DEBUGGING LINE ---
    print("DART IS SENDING (EU): ${jsonEncode(inputData)}");
    ...
    //
    final resString = await bridge.run("EU mortgage simulation", inputData); 
  ...
  void _exportReport() async {
    ...
    final reportData = ReportData(
      reportTitle: "EUR Mortgage Projection Report",
    ...
    try {
      ...
      Future.microtask(() {
        viewPdf(context, pdfBytes, 'EUR_Mortgage_Report_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf');
      });
  ...
  Widget _buildSummaryCard() {
    ...
    final percentFormatter = NumberFormat.percentPattern('de_DE');
  ...
  Widget _buildScheduleTable() {
    ...
    if (_isMonthlyView) {
      columns = const [
        ...
        DataColumn(label: Text('Payment (€)', style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.right), numeric: true),
        DataColumn(label: Text('Principal (€)', style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.right), numeric: true),
        DataColumn(label: Text('Interest (€)', style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.right), numeric: true),
        DataColumn(label: Text('Balance (€)', style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.right), numeric: true),
      ];
    } else {
       columns = const [
            ...
            SingleChildScrollView(
              ...
              child: DataTable(
                ...
                rows: data.map((item) {
                  dynamic row;
                  if (_isMonthlyView) {
                    row = item as MonthlyScheduleRow;
                  } else {
                    row = item as YearlyScheduleRow;
                  }
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            const Text("EUR Mortgage Calculator", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildTextField(loanController, 'Loan Amount (€)', icon: Icons.euro),
